...
def load_model(model_dir, nm):
    ...
    def cuda_is_available():
        # Only use the GPU path when onnxruntime exposes the CUDA provider —
        # otherwise a CPU-only onnxruntime build runs with GPU run-options and
        # crashes on arena shrinkage ("gpu:0").
        try:
