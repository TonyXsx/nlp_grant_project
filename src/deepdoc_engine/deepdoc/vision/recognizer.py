...
class Recognizer(object):
    def __init__(self, label_list, task_name, model_dir=None):
        ...
        def cuda_is_available():
            # Only use the GPU path when onnxruntime itself exposes the CUDA
            # provider — otherwise a CPU-only onnxruntime build would run with
            # GPU run-options and crash on arena shrinkage ("gpu:0").
            try:
