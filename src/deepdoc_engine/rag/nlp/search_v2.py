...
from deepdoc_engine.rag.settings import TAG_FLD, PAGERANK_FLD
from deepdoc_engine.rag.utils import rmSpace
from deepdoc_engine.rag.nlp import rag_tokenizer, query
import numpy as np
from deepdoc_engine.rag.utils.doc_store_conn import DocStoreConnection, MatchDenseExpr, FusionExpr, OrderByExpr
from deepdoc_engine.rag.nlp.model import generate_embedding, rerank_similarity
...
class Dealer:
    ...
    def retrieval(self, question, embd_mdl, tenant_ids, kb_ids, page, page_size, similarity_threshold=0.1,
        ...
        # 前 RERANK_PAGE_LIMIT 页才做 rerank,后续翻页直接按召回顺序返回
        if page <= RERANK_PAGE_LIMIT and sres.total > 0:
            logging.debug("rerank model...")
